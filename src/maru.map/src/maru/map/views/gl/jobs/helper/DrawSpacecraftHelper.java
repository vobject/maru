...
//        painter.endLine();
        ...
        FlatMapPosition elementPos = getMapPosition(element.getCurrentCoordinate());

        int pointCount = mapSettings.getVisibilityCirclePoints();
        List<GeodeticPoint> points = element.getVisibilityCircle(pointCount);

        List<FlatMapPosition> projectedCircle = new ArrayList<>(points.size());
        for (GeodeticPoint point : points) {
            projectedCircle.add(getMapPosition(point));
        }

        new DrawVisibilityCircle(gl, textRenderer, mapParams, color, 0.25).draw(elementPos, projectedCircle);



//        for (GeodeticPoint point : points) {
//            FlatMapPosition projectedPoint = getMapPosition(point);
//            painter.drawPoint(projectedPoint.X, projectedPoint.Y, 2.0, color, 0.25, true);
//        }
